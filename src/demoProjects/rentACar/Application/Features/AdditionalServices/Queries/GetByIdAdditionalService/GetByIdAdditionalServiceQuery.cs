...
namespace Application.Features.AdditionalServices.Queries.GetByIdAdditionalService
{
    public class GetByIdAdditionalServiceQuery : IRequest<AdditionalServiceDto>
    {
        ...
        public class GetByIdAdditionalServiceQueryHandler : IRequestHandler<GetByIdAdditionalServiceQuery, AdditionalServiceDto>
        {
            private readonly IAdditionalServiceRepository _additionalServiceRepository;
            private readonly IMapper _mapper;
            private readonly AdditionalServiceBusinessRules _additionalServiceBusinessRules;

            public GetByIdAdditionalServiceQueryHandler(IAdditionalServiceRepository additionalServiceRepository, IMapper mapper, AdditionalServiceBusinessRules additionalServiceBusinessRules)
            {
            ...
            public async Task<AdditionalServiceDto> Handle(GetByIdAdditionalServiceQuery request, CancellationToken cancellationToken)
            {
                AdditionalService? additionalService = await _additionalServiceRepository.GetAsync(a => a.Id == request.Id);
