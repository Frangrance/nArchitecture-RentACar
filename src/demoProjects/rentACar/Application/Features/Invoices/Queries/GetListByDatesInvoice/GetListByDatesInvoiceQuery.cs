...
public class GetListByDatesInvoiceQuery : IRequest<InvoiceListModel>
{
    ...
    public class GetListInvoiceByDatesQueryHandler : IRequestHandler<GetListByDatesInvoiceQuery, InvoiceListModel>
    {
        ...
        public async Task<InvoiceListModel> Handle(GetListByDatesInvoiceQuery request, CancellationToken cancellationToken)
        {
            IPaginate<Invoice> invoices = await _invoiceRepository.GetListAsync(i => i.CreatedDate >= request.StartDate && i.CreatedDate <= request.EndDate,
                                                   include: i => i.Include(i => i.Customer).Include(i => i.Customer.IndividualCustomer).Include(i => i.Customer.CorporateCustomer),
