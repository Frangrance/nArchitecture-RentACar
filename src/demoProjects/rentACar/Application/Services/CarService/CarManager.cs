...
namespace Application.Services.CarService
{
    public class CarManager : ICarService
    {
        ...
        {
            IList<Car> cars = (await _carRepository.GetListAsync(c => c.ModelId == modelId && c.RentalBranchId == rentStartRentalBranch)).Items;
        ...
        public async Task<Car> GetById(int id)
        {
            ...
            if (car is null) throw new BusinessException("The car doesn't exist.");
            return car;
        }

        public async Task<Car> PickUpCar(Rental rental)
        {
            Car carToBeUpdate = await _carRepository.GetAsync(c => c.Id == rental.Id);
